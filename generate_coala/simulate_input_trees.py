    ...
    def post_process_tgl_files(self):
        ...
        def parse_tgl_file(tgl_path):
            with open(tgl_path, "r") as file:
                content = file.read()
                # Normalize decimal format to use '.' instead of ','
...
class InferParasiteBranches:
    ...
    def infer_parasite_times(p_tree, host_times, mapping):
        ...
        # Rescale parasite times to match host tree depth
        max_p_time = max(times.values())
        max_h_time = max(host_times.values())
        scaling_factor = max_h_time / max_p_time if max_p_time > 0 else 1.0
        for clade in times:
            times[clade] *= scaling_factor
        return times
