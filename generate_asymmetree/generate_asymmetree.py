...
for i in range(num_trees):
    ...
    g = te.dated_gene_tree(s, dupl_rate=0.7, loss_rate=0.645, hgt_rate=hgt_rate) 
    s_nwk = to_newick(s)
    g_nwk = to_newick(g)
...
for i in range(num_trees):
    ...
    hgt_rate = random.uniform(2, 2.5)
    g = te.dated_gene_tree(s, dupl_rate=0.7, loss_rate=0.66, hgt_rate=hgt_rate) 
    s_nwk = to_newick(s)
    g_nwk = to_newick(g)
    s_nwk = re.sub(r'(?<=[(,)])(?!(H))(\d+):', r'H\2:', s_nwk)
    g_nwk = gene_pattern.sub(lambda m: f"P{m.group(1)}", g_nwk)
...
for i in range(num_trees):
    ...
    hgt_rate = random.uniform(1.5, 1.8)
    g = te.dated_gene_tree(s, dupl_rate=0.7, loss_rate=0.6, hgt_rate=hgt_rate) 
    s_nwk = to_newick(s)
    g_nwk = to_newick(g)
    s_nwk = re.sub(r'(?<=[(,)])(?!(H))(\d+):', r'H\2:', s_nwk)
    g_nwk = gene_pattern.sub(lambda m: f"P{m.group(1)}", g_nwk)
